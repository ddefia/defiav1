...
import { loadAutomationSettings, saveAutomationSettings, loadIntegrationKeys, saveIntegrationKeys, getBrandRegistryEntry, saveBrandRegistryEntry, forceSeedDefaultBrands, resetBrandToDefault, inviteTeamMember, fetchTeamMembers, removeTeamMember } from '../services/storage';
...
export type SettingsTab = 'general' | 'notifications' | 'security' | 'billing' | 'team';
...
export const Settings: React.FC<SettingsProps> = ({ brandName, config, onChange, onNavigateToBrandKit, theme = 'dark', onThemeChange, initialTab }) => {
    ...
    const [teamInviteRole, setTeamInviteRole] = useState('editor');
    ...
    useEffect(() => {
        ...
        const loadTeam = async () => {
            ...
            try {
                const registryEntry = getBrandRegistryEntry(brandName);
                const brandKey = registryEntry?.brandId || brandName;
                const result = await fetchTeamMembers(brandKey);
                if (isActive) {
                    setTeamMembers(result.members);
                    setTeamOwnerId(result.owner_id);
                }
            } catch {
                if (isActive) setTeamError('Failed to load team members');
            } finally {
    ...
    const handleInviteTeamMember = async () => {
        ...
        try {
            const registryEntry = getBrandRegistryEntry(brandName);
            const brandKey = registryEntry?.brandId || brandName;
            const result = await inviteTeamMember(brandKey, teamInviteEmail.trim(), teamInviteRole);
            if (result.success) {
                setTeamSuccess(`Invited ${teamInviteEmail.trim()}`);
                setTeamInviteEmail('');
                // Refresh the list
                const refreshed = await fetchTeamMembers(brandKey);
                setTeamMembers(refreshed.members);
            } else {
                setTeamError(result.error || 'Failed to invite');
            }
        } catch (e: any) {
    ...
    const handleRemoveTeamMember = async (memberId: string) => {
        ...
        try {
            const registryEntry = getBrandRegistryEntry(brandName);
            const brandKey = registryEntry?.brandId || brandName;
            const success = await removeTeamMember(brandKey, memberId);
            if (success) {
                ...
                <div className="w-[220px] flex flex-col gap-1 flex-shrink-0">
                    {tabs.map(tab => (
                        <button
                            ...
                            style={tab.disabled ? { color: 'var(--text-faint)' } : activeTab !== tab.id ? { color: 'var(--text-secondary)' } : undefined}
                        >
                            <span
                ...
                <div className="flex-1 flex flex-col gap-6 overflow-y-auto">
                    ...
                    {activeTab === 'team' && (
                        <div className="flex flex-col gap-6">
                            ...
                            <div className="rounded-[14px] p-6" style={{ backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border)', boxShadow: 'var(--card-shadow)' }}>
                                <div className="flex items-center gap-3 mb-2">
                                    <span className="material-symbols-sharp text-[#FF5C00] text-xl" style={{ fontVariationSettings: "'wght' 300" }}>person_add</span>
                                    <span className="text-base font-semibold" style={{ color: 'var(--text-primary)' }}>Invite Team Member</span>
                                </div>
                                <p className="text-xs mb-5" style={{ color: 'var(--text-muted)' }}>
                                    Invite people by email to access this brand dashboard. They'll see the brand when they log in.
                                </p>
                                ...
                                {teamError && (
                                    <p className="text-xs text-red-400 mt-3">{teamError}</p>
                                )}
                                {teamSuccess && (
                                    <p className="text-xs text-green-400 mt-3">{teamSuccess}</p>
                                )}
                            </div>
                            ...
                            <div className="rounded-[14px] p-6" style={{ backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border)', boxShadow: 'var(--card-shadow)' }}>
                                <div className="flex items-center gap-3 mb-5">
                                    ...
                                    <span className="text-base font-semibold" style={{ color: 'var(--text-primary)' }}>Team Members</span>
                                ...
                                {teamLoading ? (
                                    <div className="flex items-center gap-2 py-4" style={{ color: 'var(--text-muted)' }}>
                                        <span className="material-symbols-sharp text-lg animate-spin" style={{ fontVariationSettings: "'wght' 300" }}>progress_activity</span>
                                        <span className="text-sm">Loading team...</span>
                                    </div>
                                ) : teamMembers.length === 0 ? (
                                    <div className="py-6 text-center">
                                        <span className="material-symbols-sharp text-3xl mb-2 block" style={{ color: 'var(--text-faint)', fontVariationSettings: "'wght' 100" }}>group_add</span>
                                        <p className="text-sm" style={{ color: 'var(--text-muted)' }}>No team members yet. Invite someone above.</p>
                                    </div>
                                ) : (
                                    <div className="space-y-2">
                                        {/* Owner row */}
                                        {userProfile && teamOwnerId === userProfile.id && (
                                            <div className="flex items-center gap-3 px-4 py-3 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
                                                <div className="w-9 h-9 rounded-full bg-[#FF5C00] flex items-center justify-center flex-shrink-0">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <p className="text-sm font-medium truncate" style={{ color: 'var(--text-primary)' }}>
                                                        {userProfile.fullName || userProfile.email} <span className="text-xs font-normal" style={{ color: 'var(--text-muted)' }}>(you)</span>
                                                    </p>
                                                    <p className="text-xs truncate" style={{ color: 'var(--text-muted)' }}>{userProfile.email}</p>
                                                </div>
                                                <span className="text-[10px] font-medium uppercase px-2 py-1 rounded" style={{ backgroundColor: '#FF5C00', color: 'white' }}>Owner</span>
                                            </div>
                                        )}

                                        {teamMembers.map(member => (
                                            <div key={member.id} className="flex items-center gap-3 px-4 py-3 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
                                                <div className="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0" style={{ backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border)' }}>
                                                ...
                                                <span
                                                    ...
                                                    style={{
                                                        backgroundColor: member.status === 'pending' ? '#FFA50020' : '#22C55E20',
                                                        color: member.status === 'pending' ? '#FFA500' : '#22C55E',
                                                    }}
                                                >
                                                ...
                                                {teamOwnerId === userProfile?.id && (
                                                    <button
                                                        ...
                                                        title="Remove member"
                                                    >
                                                        <span className="material-symbols-sharp text-lg text-red-400" style={{ fontVariationSettings: "'wght' 300" }}>delete</span>
                                                    </button>
                                                )}
                                            </div>
                                        ))}
                            ...
                            <div className="rounded-[14px] p-5" style={{ backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border)', boxShadow: 'var(--card-shadow)' }}>
                                <div className="flex items-start gap-3">
                                    <span className="material-symbols-sharp text-lg mt-0.5" style={{ color: '#FF5C00', fontVariationSettings: "'wght' 300" }}>info</span>
                                    <div>
                                        <p className="text-sm font-medium mb-1" style={{ color: 'var(--text-primary)' }}>How team access works</p>
                                        <ul className="text-xs space-y-1" style={{ color: 'var(--text-muted)' }}>
                                            <li>Team members see this brand in their sidebar when they log in</li>
                                            <li><strong>Editors</strong> can view and edit brand content, calendar, and campaigns</li>
                                            <li><strong>Viewers</strong> can see the dashboard and content but cannot make changes</li>
                                            <li>Only the brand owner can invite or remove team members</li>
                                        </ul>
                                    </div>
                                </div>
                            </div>
