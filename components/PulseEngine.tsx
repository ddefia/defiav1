...
import { generateTrendReaction, generateWeb3Graphic } from '../services/gemini';
...
export const PulseEngine: React.FC<PulseEngineProps> = ({ brandName, brandConfig, onLaunchCampaign, onSchedule }) => {
    ...
    // Filters & Inputs
    const [sourceFilter, setSourceFilter] = useState<'All' | 'Twitter' | 'News' | 'OnChain'>('All');
    ...
    useEffect(() => {
        ...
        const shouldFetch = (now - cache.lastUpdated) > TWENTY_MINS || cache.items.length === 0;
        ...
        intervalRef.current = setInterval(() => {
            ...
            if ((currentNow - currentCache.lastUpdated) > TWENTY_MINS) {
                handleScan();
            }
        }, TWENTY_MINS);
            ...
            <div className="w-full lg:w-1/3 flex flex-col gap-4">
                <div className="bg-white p-4 rounded-xl border border-brand-border shadow-sm">
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                        {(['All', 'Twitter', 'OnChain', 'News'] as const).map(filter => (
                            <button
                                key={filter}
                        ...
                        <div className="border-b border-brand-border pb-6 mb-6">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <span className={`px-2 py-1 text-[10px] font-bold uppercase rounded border ${
                                    selectedTrend.sentiment === 'Positive' ? 'bg-green-50 text-green-700 border-green-200' : 
