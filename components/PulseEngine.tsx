...
export const PulseEngine: React.FC<PulseEngineProps> = ({ brandName, brandConfig, onLaunchCampaign, onSchedule }) => {
    ...
    const getSourceIcon = (source: string) => {
        ...
        if (source === 'LunarCrush') return <span className="text-purple-400">🌕</span>; // Lunar Icon
                ...
                <div className="bg-white p-4 rounded-xl border border-brand-border shadow-sm">
                    ...
                    <div className="mt-4 pt-4 border-t border-brand-border">
                        <Button
                            ...
                            disabled={isAnalyzingBusiness || trends.length === 0}
                            className="w-full text-xs"
                            variant="secondary"
                        >
                            {isAnalyzingBusiness ? "Connecting dots..." : "✨ Analyze Business Opportunities"}
                        </Button>
                        {businessIdeas && (
                            <div className="mt-4 bg-indigo-50 p-4 rounded-lg border border-indigo-100 text-sm max-h-60 overflow-y-auto custom-scrollbar">
                                <div className="flex justify-between items-center mb-2">
                                    <h4 className="font-bold text-indigo-900">Strategic Insights</h4>
                                    <button onClick={() => setBusinessIdeas('')} className="text-xs text-indigo-400 hover:text-indigo-600">Close</button>
                                </div>
                                <div className="prose prose-sm prose-indigo max-w-none">
                                    <pre className="whitespace-pre-wrap font-sans text-indigo-800">{businessIdeas}</pre>
                                </div>
                            </div>
                        )}
