...
import { fetchMentions, fetchCompetitorTweets, computeSocialSignals, fetchSocialMetrics, computeGrowthMetrics, autoGenerateDuneQueries } from './services/analytics';
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        const runBackgroundScan = async () => {
            ...
            try {
                ...
                // Analytics report is generated by server morning cron (6 AM daily) and loaded from cloud above.
                // Removed client-side generateAnalyticsReport() call — it burned 1 Gemini call every 3h per user.
                // If report is missing, the server cron will populate it at 6 AM.
                ...
                setSystemLogs(prev => [`GAIA: Daily Briefing age: ${hoursOld}h. ${!currentReport ? 'Will be populated by server cron.' : 'Loaded.'}`, ...prev]);
                ...
                const competitorHandles = (profiles[selectedBrand]?.competitors || [])
                    .map((c: any) => c.handle).filter(Boolean);
                const competitorTweets = competitorHandles.length > 0
                    ? await fetchCompetitorTweets(competitorHandles).catch(e => { console.warn("Competitor tweets failed", e); return []; })
                    : [];
                if (competitorTweets.length > 0) {
                    setSystemLogs(prev => [`Intel: Loaded ${competitorTweets.length} competitor tweets from ${competitorHandles.length} competitor(s)`, ...prev]);
                }

                const { analysis, actions } = await orchestrateMarketingDecision(brainContext, { calendarEvents, mentions, competitorTweets });
    ...
    // Auto-generate recommendations on brand load if stale (>24h) or empty
    // 24h threshold: server cron runs hourly and populates Supabase cache.
    // Client auto-regen is only a fallback for when the cron missed a full day.
    // DO NOT lower this — even 2h meant 6 client-side Gemini calls every session.
    useEffect(() => {
        if (!selectedBrand || !profiles[selectedBrand]) return;
